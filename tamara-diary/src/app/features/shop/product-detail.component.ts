...
@Component({
  ...
  <div *ngIf="product() as p" class="product">
    <div class="images">
      <img *ngFor="let img of p.images; let i = index" [src]="img" [alt]="(p.title | translate) + ' ' + (i+1)" />
    </div>
    <div class="info">
  ...
  <ng-container *ngIf="p.id === 'card-pack'; else normalOrder">
    <a [routerLink]="['/gallery']" [queryParams]="{ target: 'card-pack' }" class="btn">{{ 'product.view' | translate }}</a>
    <a [routerLink]="['/order']" [queryParams]="{ pid: p.id, ptitle: (p.title | translate) }" class="btn btn-primary">{{ 'product.orderThis' | translate }}</a>
  </ng-container>
